...
var svg = 'maps/microrregioes.svg';
...
function onSVGLoaded(data) {
      ...
      if(municipios = mapa.select('#Microrregioes')) {
            $.each(municipios.selectAll("path").items, function () {
                  this.attr({ 'style' :  "fill:#EEDDB3" });
                  this.hover(
                        () => {
                              let slicedId1 = this.attr('id').slice(4, 7);
                              let slicedId2 = this.attr('id').slice(8, 11);
                              let slicedId = slicedId1 + slicedId2;
                              let jsondata = fetch(url + slicedId)
                                    .then(res => res.json())
                                    .then(data => jsonResponse = data)
                                    .then(() => this.append(Snap.parse('<title>Microrregião: ' + jsonResponse.nome
                  ...
                  this.click(() => {
                        ...
                        let jsondata = fetch(url + slicedId)
                              .then(res => res.json())
                              .then(data => jsonResponse = data)
                              .then(() => this.append(alert('Dados da microrregião ' + jsonResponse.nome 
                                    + ' de ID '+ slicedId + ':')));
                  });
            });
      }
